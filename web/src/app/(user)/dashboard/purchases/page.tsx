        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          {orders.length > 0 && (
            <div className="relative group">
              <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground group-hover:text-blue-500 transition-colors" />
              <input 
                type="text"
                placeholder="Search purchases..."
                className="pl-10 pr-4 h-11 bg-background border border-border rounded-xl text-sm font-medium focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all w-full md:w-64"
              />
            </div>
          )}
        </div>
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="flex items-start md:items-center gap-5">
                  <div className="w-16 h-16 rounded-2xl bg-secondary/50 border border-border flex items-center justify-center shrink-0">
                    <Package className="w-7 h-7 text-muted-foreground group-hover:text-blue-600 transition-colors" />
                  </div>
                  
                  <div>
                    <Link href={`/orders/${order.id}`} className="hover:underline decoration-2 underline-offset-4 decoration-blue-500">
                      <h3 className="text-lg font-bold text-foreground group-hover:text-blue-700 transition-colors">
                      ...
                      <Badge
                        ...
                        className={`text-[11px] font-bold uppercase tracking-wider px-2 py-0.5 ${
                          order.status === "completed"
                            ? "bg-success/10 text-success border-emerald-100/50"
                ...
                <div className="flex md:flex-col items-center md:items-end justify-between md:justify-center border-t md:border-t-0 border-border pt-4 md:pt-0">
                  ...
                  {order.github_repo_url ? (
                    <a
                      ...
                      className="inline-flex items-center justify-center gap-2 h-10 px-4 rounded-xl bg-primary text-primary-foreground text-sm font-bold hover:bg-slate-800 transition-colors shadow-sm"
                    >
                    ...
                    <Link 
                      ...
                      className="inline-flex items-center justify-center gap-2 h-10 px-4 rounded-xl bg-secondary text-foreground text-sm font-bold hover:bg-slate-200 transition-colors"
                    >
