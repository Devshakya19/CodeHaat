...
CREATE POLICY "Own transactions" ON wallet_transactions FOR SELECT USING (
...
CREATE POLICY "Mark own notifications" ON notifications FOR UPDATE USING (auth.uid() = user_id);
